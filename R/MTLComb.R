...
#' @param stratify True/False indicator to tell if stratified cv is used
...
MTLComb_CV = function(X=NULL, Y=NULL, nfolds=10, lam_ratio=0.01, nlambda=10, lambda=NULL, ctasks=NULL, intercept=F, 
                           opts=list(init=0, maxIter=50, tol=0.001, ter=2), C2=0, C=0, stratify=FALSE){
  library("pROC")
  
  #partition the subjects for CV
  getCVPartition <- function(Y, cv_fold, ctasks = 1:5, stratify = TRUE){
    task_num <- length(Y)
    randIdx  <- lapply(Y, function(x) sample.int(length(x), length(x), replace = FALSE))
    
    cvPar <- vector("list", cv_fold)
    
    for (cv_idx in seq_len(cv_fold)){
      cvTrain <- vector("list", task_num)
      cvTest  <- vector("list", task_num)
      
      for (t in seq_len(task_num)){
        n <- length(Y[[t]])
        
        use_stratify <- stratify && (t %in% ctasks)
        
        if (use_stratify){
          # default binary labels: -1 / +1
          ct <- which(Y[[t]][randIdx[[t]]] == -1)
          cs <- which(Y[[t]][randIdx[[t]]] ==  1)
          
          te_pos <- c(
            ct[seq(cv_idx, length(ct), by = cv_fold)],
            cs[seq(cv_idx, length(cs), by = cv_fold)]
          )
          te_pos <- sort(te_pos)
        } else {
          te_pos <- seq(cv_idx, n, by = cv_fold)
        }
        
        tr_pos <- setdiff(seq_len(n), te_pos)
        
        cvTrain[[t]] <- randIdx[[t]][tr_pos]
        cvTest[[t]]  <- randIdx[[t]][te_pos]
      }
  ...
  cvPar <- getCVPartition(Y, nfolds, ctasks=ctasks, stratify=stratify)
