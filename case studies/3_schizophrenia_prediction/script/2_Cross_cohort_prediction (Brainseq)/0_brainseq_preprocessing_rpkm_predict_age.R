...
rse_gene <- rse_gene[, rse_gene$Dx == "Control"]
...
rpkm_aggregated <- rpkm_aggregated[!duplicated(rownames(rpkm_aggregated)), ] #<- difference?
...
dim(normalized_counts) #18291   286
...
#run sva
covariates= c( "RIN", "Race", "Sex", cells)
str(meta[, c("Age", covariates)])
mod = model.matrix(~Age + as.numeric(RIN) + Race + Sex + Astrocytes + Oligodendrocytes + Microglia + Endothelial + Neurons + OPC, 
                   data = meta)
mod0 = model.matrix(~1 + as.numeric(RIN) + Race+ Sex + Astrocytes + Oligodendrocytes + Microglia + Endothelial + Neurons + OPC, 
...
#residualize data
covariates <- meta[, c("Race", "RIN","Sex", paste0("sv", 1:5), paste0("PC", 1:5),cells)] #<- no pc?
...
saveRDS(corr_bs, file="../../preprocessed_data/Brainseq/gene_expression_predict_age.rds")
saveRDS(as.data.frame(meta[,c("sex", "age", "diagnosis")]), "../../preprocessed_data/Brainseq/y_matrix_predict_age.rds" )



brainseq_meta=read.table("../../raw_data/phs000979.v1.p1.txt", header = TRUE, sep = "\t")
hbcc_meta=read.table("../../raw_data/phs000979.v2.p2.txt", header = TRUE, sep = "\t")
brainseq_meta = brainseq_meta[!is.element(brainseq_meta$dbgap_subject_id, hbcc_meta$dbgap_subject_id),]
length(unique(brainseq_meta$SUBJECT_ID))
brainseq_meta = brainseq_meta[brainseq_meta$molecular_sample_use==names(table(brainseq_meta$molecular_sample_use)[1]), ]
length(unique(brainseq_meta$SUBJECT_ID))
