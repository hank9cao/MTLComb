...
metaExp <- metaExp[metaExp$Diagnosis %in% c("Schizo", "Control"),] 	#extract schizophrenia and control subjects
metaExp <- metaExp[metaExp$Race %in% c("African American","Caucasian"),]
metaExp$Diagnosis <- as.character(metaExp$Diagnosis)
metaExp$Diagnosis[metaExp$Diagnosis=="Schizo"] <- as.numeric(1)
metaExp$Diagnosis[metaExp$Diagnosis=="Control"] <- as.numeric(0)
metaExp=metaExp[!is.na(metaExp$AgeDeath), ]# remove 33
metaExp=metaExp[metaExp$AgeDeath>=18, ]
metaExp=metaExp[metaExp$AgeDeath<=65, ]
metaExp[metaExp$Smoker %in% "not filled in","Smoker"]="Unknown"
metaExp$Race = as.character(metaExp$Race)
...
#sva analysis
age2=metaExp$AgeDeath^2
metaExp <- cbind(metaExp,age2)

mod = model.matrix(~as.factor(Diagnosis)+as.numeric(pH)+as.numeric(RIN)+as.numeric(PMI)+as.factor(Race) +as.factor(Sex)+AgeDeath+age2, data=metaExp)
mod0 = model.matrix(~as.numeric(pH)+as.numeric(RIN)+as.numeric(PMI)+as.factor(Race)+as.factor(Sex)+AgeDeath+age2+1, data=metaExp)
...
#residualize
covariates= metaExp[, c("pH", "RIN", "PMI", "Race", "Sex", "AgeDeath", "age2", paste0("sv", 1:5), paste0("PC", 1:5))]
...
saveRDS(all_res, file="../../preprocessed_data/HBCC/gene_expression_predict_Dx.rds")
saveRDS(metaExp[,c("SAMPLE_ID", "sex", "age", "diagnosis")], "../../preprocessed_data/HBCC/y_matrix_predict_Dx.rds" )
